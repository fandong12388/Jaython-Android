...
import com.tiny.volley.core.HttpHeaderParser;
...
import com.tiny.volley.utils.RequestHelper;
...
public class HttpGsonRequest<T> extends BaseRequest<T> {
    ...
    public HttpResponse<T> parseNetworkResponse(NetworkResponse response) {
        ...
        try {
            ...
            if (getUrl().startsWith(TinyVolley.sHost)) {
            ...
            if (!TextUtils.isEmpty(data)) {
                //1.首先解析占位符
                if (null != TinyVolley.sPlaceholderParser) {
                    data = TinyVolley.sPlaceholderParser.parsePlaceholderJson(data);
                }
                //2.解析器首先解析
                if (mParser != null) {
