 ...
 * @author fandong
 */
public class CollectNewsFragment extends BaseFragment<NewsModel> {


    public static final String BLOCK_NEWS = "block_news";
    public static final String BLOCK_NEWS_KEY = "block_collect_news_key";
    ...
    public View createView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.frag_collect_news, container, false);
    }
    ...
    private void initCache() {
        ...
        if (mCache != null) {
            JSONArray array = mCache.getAsJSONArray(BLOCK_NEWS_KEY + LoginManager.getInstance().getUid());
            if (null != array) {
    ...
    private void refresh() {
        ...
        Subscription subscription = mViewModel.refreshCollect()
                .subscribe(this::onRefreshNext, this::onError);
        addSubscription(subscription);
    }

    private void loadMore() {
        Subscription subscription = mViewModel.loadMoreCollect()
    ...
    private void onRefreshNext(List<NewsItem> items) {
        if (ValidateUtil.isValidate(items)) {
            ...
            mCache.put(BLOCK_NEWS_KEY + LoginManager.getInstance().getUid(), GsonUtil.toJson(items));
        }
        if (mAdapter.getItemCount() > 0) {
            showContentView();
        } else {
