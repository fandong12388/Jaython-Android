...
public class SwipeBackLayout extends FrameLayout {
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        float edgeRatio = a.getFloat(R.styleable.SwipeBackLayout_edge_ratio, 0.3f);
     ...
     * @param ratio 比例
     */
    public void setEdgeRatio(float ratio) {
        if (ratio < 0.f || ratio > 1.f) {
            throw new IllegalArgumentException("Ratio must > =0.f and <= 1.f");
        }
        int width = getContext().getResources().getDisplayMetrics().widthPixels;
        int pixel = Math.round(ratio * width);
        mDragHelper.setEdgeSize(pixel);
    }
