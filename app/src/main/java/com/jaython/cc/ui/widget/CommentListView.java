...
import com.jaython.cc.bean.CommentItem;
...
public class CommentListView extends LinearLayout {
    ...
    public void setDatas(List<CommentItem> data) {
        if (data == null) {
    ...
    private View getView(final int position) {
        ...
        String name = bean.getUser().getUserProfile().getNickname();
        String id = bean.getId();
        String toReplyName = "";
        if (bean.getToReplyUser() != null) {
            toReplyName = bean.getToReplyUser().getUserProfile().getNickname();
        }

        SpannableStringBuilder builder = new SpannableStringBuilder();
        builder.append(setClickableSpan(name, bean.getUser().getUid()));

        if (!TextUtils.isEmpty(toReplyName)) {

            builder.append(" 回复 ");
            builder.append(setClickableSpan(toReplyName, bean.getToReplyUser().getUid()));
        }
        builder.append(": ");
        //转换表情字符
        String contentBodyStr = bean.getContent();
        builder.append(SystemUtil.formatUrlString(contentBodyStr));
    ...
    private SpannableString setClickableSpan(final String textStr, final String id) {
        ...
        subjectSpanText.setSpan(new SpannableClickable(itemColor) {
                                    ...
                                    public void onClick(View widget) {
                                        Toast.makeText(gContext, textStr + " &id = " + id, Toast.LENGTH_SHORT).show();
                                    }
